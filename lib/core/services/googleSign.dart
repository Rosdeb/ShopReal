...
class GoogleSignInService {
  ...
  static Future<void> _initSignIn() async {
    if (!isInitialized) {
      await _googleSignIn.initialize(
        // Web client ID — required so Android returns a usable idToken for backend OAuth.
        serverClientId:
            '216387410941-84g359m34sdqdkrot00l68q6anqrak8m.apps.googleusercontent.com',
      );
      isInitialized = true;
    }
  }
  ...
  static Future<void> _syncUserToFirestore(User user) async {
    try {
      ...
      // Firestore is not required for the app auth flow. The backend login
      // runs after Google auth, so Firestore sync should not block sign-in.
      Logger.error('Firestore user sync failed during Google sign-in', e);
    }
  }
